...
import { fetchGuideProjects } from '../../../actions/fetch-projects'
import Link from 'next/link';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import Image from 'next/image';
import { Badge } from '@/components/ui/badge';

async function GuideProjects({id} : {id: string}) {

  const projects = await fetchGuideProjects(id)  
              ...
              <CardContent className="flex flex-col gap-6 text-sm mt-2">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-foreground">{project.student.name}</p>
                    <p className="text-xs text-muted-foreground">{project.student.email}</p>
                  </div>
                </div>
